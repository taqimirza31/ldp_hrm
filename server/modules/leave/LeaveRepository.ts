...
export class LeaveRepository extends BaseRepository {
  ...
  async insertPolicy(p: any) { const r = await this.sql`INSERT INTO leave_policies(name,applicable_departments,applicable_employment_types,applicable_roles,effective_from,effective_to,policy_year,is_active,created_by) VALUES(${p.name},${JSON.stringify(p.applicableDepartments||[])},${JSON.stringify(p.applicableEmploymentTypes||[])},${JSON.stringify(p.applicableRoles||[])},${p.effectiveFrom},${p.effectiveTo||null},${p.policyYear||null},${p.isActive!==false},${p.createdBy||null}) RETURNING *` as any[]; return r[0]; }
  ...
  async insertType(t: any) { const r = await this.sql`INSERT INTO leave_types(policy_id,name,paid,accrual_type,accrual_rate,max_balance,carry_forward_allowed,max_carry_forward,requires_document,requires_approval,auto_approve_rules,hr_approval_required,min_days,max_days_per_request,blocked_during_notice,color) VALUES(${t.policyId},${t.name},${t.paid!==false},${t.accrualType||"none"},${t.accrualRate!=null?String(t.accrualRate):null},${t.maxBalance!=null?parseInt(String(t.maxBalance),10):21},${!!t.carryForwardAllowed},${t.maxCarryForward!=null?parseInt(String(t.maxCarryForward),10):null},${!!t.requiresDocument},${t.requiresApproval!==false},${t.autoApproveRules?JSON.stringify(t.autoApproveRules):null},${!!t.hrApprovalRequired},${t.minDays!=null?parseInt(String(t.minDays),10):null},${t.maxDaysPerRequest!=null?parseInt(String(t.maxDaysPerRequest),10):null},${!!t.blockedDuringNotice},${t.color||"#3b82f6"}) RETURNING *` as any[]; return r[0]; }
  ...
  async findEarnedLeaveTypeId(policyId?: string|null) { if (policyId) { const r = await this.sql`SELECT id FROM leave_types WHERE policy_id=${policyId} AND (LOWER(name) LIKE ANY(ARRAY['%earned%','%annual%']) OR LOWER(TRIM(name))='el') LIMIT 1` as any[]; return r[0]?.id??null; } const r = await this.sql`SELECT id FROM leave_types WHERE LOWER(name) LIKE ANY(ARRAY['%earned%','%annual%']) OR LOWER(TRIM(name))='el' LIMIT 1` as any[]; return r[0]?.id??null; }
  async findBereavementLeaveTypeId(policyId?: string|null) { if (policyId) { const r = await this.sql`SELECT id FROM leave_types WHERE policy_id=${policyId} AND LOWER(name) LIKE '%bereavement%' LIMIT 1` as any[]; return r[0]?.id??null; } const r = await this.sql`SELECT id FROM leave_types WHERE LOWER(name) LIKE '%bereavement%' LIMIT 1` as any[]; return r[0]?.id??null; }
  ...
  async getAllBalances() {
    return this.sql`SELECT e.id as employee_id,e.first_name,e.last_name,e.employee_id as emp_code,e.department,lt.id as leave_type_id,lt.name as type_name,lt.paid,lt.color,elb.id as id,COALESCE(elb.balance,0)::text as balance,COALESCE(elb.used,0)::text as used FROM employees e CROSS JOIN leave_types lt INNER JOIN leave_policies lp ON lp.id=lt.policy_id AND lp.name='Standard Leave Policy' LEFT JOIN LATERAL (SELECT id,balance,used FROM employee_leave_balances WHERE employee_id=e.id AND leave_type_id=lt.id ORDER BY updated_at DESC NULLS LAST LIMIT 1) elb ON true WHERE e.employment_status='active' ORDER BY e.last_name,e.first_name,lt.name` as Promise<any[]>;
  }
