...
export class LeaveController {
  ...
  async createPolicy(_: Request, res: Response) { res.status(403).json({ error: "Only the standard leave policy is supported. Run migration 0021_standard_leave_policy.sql if the policy is missing." }); }
  ...
  async createType(_: Request, res: Response) { res.status(403).json({ error: "Only three leave types are supported: Earned Leave, LWOP, and Bereavement." }); }
  ...
  async processYearEnd(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      const result=await this.svc.processYearEnd(year,req.user?.employeeId??req.user?.id??null);
